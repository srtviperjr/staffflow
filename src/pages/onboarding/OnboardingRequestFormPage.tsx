...
import { DEPARTMENTS, ROLES, type OnboardingFormData } from '../../types/onboarding'
...
export default function RequestFormPage() {
  const { addRequest } = useOnboardingRequests()
  ...
  const validate = () => {
    const nextErrors: Partial<Record<keyof OnboardingFormData, string>> = {}
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth error={Boolean(errors.role)} required>
                  ...
                  <Select
                    label="Role"
                    value={form.role}
                    onChange={(e) => updateField('role', e.target.value as OnboardingFormData['role'])}
                  >
                    {ROLES.map((role) => (
              ...
              <Grid size={12}>
                <FormControl fullWidth error={Boolean(errors.department)} required>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      updateField('department', e.target.value as OnboardingFormData['department'])
                    }
                  >
